    ...
    atoms_with_spinnable_Hs &spinables)
{
    ...
    for (unsigned int i = 0; i < alt_confs.size(); i++) {
        ...
        if (have_1) {
            ...
            gemmi::Atom h1;
            h1.name = at_name_1;
            h1.element = gemmi::Element("H");
            h1.pos = gemmi::Position(pav_1.x(), pav_1.y(), pav_1.z());
            h1.occ = 1.0f;
            h1.b_iso = bf;
            if (!alt_confs[i].empty()) h1.altloc = alt_confs[i][0];

            gemmi::Atom h2;
            h2.name = at_name_2;
            h2.element = gemmi::Element("H");
            h2.pos = gemmi::Position(pav_2.x(), pav_2.y(), pav_2.z());
            h2.occ = 1.0f;
            h2.b_iso = bf;
            if (!alt_confs[i].empty()) h2.altloc = alt_confs[i][0];

            gemmi::Atom h3;
            h3.name = at_name_3;
            h3.element = gemmi::Element("H");
            h3.pos = gemmi::Position(pav_3.x(), pav_3.y(), pav_3.z());
            h3.occ = 1.0f;
            h3.b_iso = bf;
            if (!alt_confs[i].empty()) h3.altloc = alt_confs[i][0];
