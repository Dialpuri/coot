...
#include "/lmb/home/jdialpuri/Development/coot-tooling/generated-tests/coot__centre_of_molecule_using_masses/gemmi/function.hh"

namespace coot {

std::string element_to_padded_string(const std::string& elem_name) {
    if (elem_name.empty())
        return "  ";
    std::string result = elem_name;
    if (result.size() == 1)
        return " " + result;
    if (result.size() == 2)
        return result;
    // For longer names, take first 2 chars and pad
    return result.substr(0, 2);
}

std::pair<bool, double>
radius_of_gyration_gemmi(const gemmi::Structure& st) {
   ...
   double sum_dd = 0.0;
   auto centre_pair = coot::centre_of_molecule_using_masses_gemmi(st);
         ...
         for (const gemmi::Chain& chain : model.chains) {
            for (const gemmi::Residue& residue : chain.residues) {
               for (const gemmi::Atom& atom : residue.atoms) {
                  std::string ele = element_to_padded_string(atom.element.name());
                  double w = 14.0;
