    ...
    for (char alt : alt_confs) {
        ...
        if (tors < -90.0 || tors > 90.0) {
            ...
            if (tors2 >= -90.0 && tors2 <= 90.0) {
                ...
                if (apply_swap) {
                    // Replicate MMDB SwapAtoms() behavior: COPY, not swap
                    CD1->pos = CD2->pos;
                    CD1->name = CD2->name;
                }
                if (CE1 && CE2) {
...
int test_and_fix_ASP_GLU_nomenclature_errors_gemmi(gemmi::Residue &residue, bool apply_swap) {
    ...
    for (char alt : alt_confs) {
        ...
        if (tors < -90.0 || tors > 90.0) {
            ...
            if (i1>=0 && i2>=0) {
                if (apply_swap) {
                    // Replicate MMDB SwapAtoms(): COPY, not swap
...
std::vector<gemmi::CRA> fix_gemmi(gemmi::Model &m, const coot::protein_geometry *geom_p) {
    std::vector<gemmi::CRA> fixed_residues;
    for (auto &chain : m.chains) {
        for (auto &residue : chain.residues) {
            gemmi::CRA cra{&chain, &residue, nullptr};
            if (detail::test_and_fix_PHE_TYR_nomenclature_errors_gemmi(cra, true) > 0)
                fixed_residues.push_back(cra);
            if (detail::test_and_fix_ASP_GLU_nomenclature_errors_gemmi(residue, true) > 0)
                fixed_residues.push_back(cra);
        }
    }
