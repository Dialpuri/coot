...
public:
  ...
  // MMDB original:
  //   dictionary_residue_restraints_t(mmdb::Manager *mol) {
  //     find first residue → init(residue_p)
  //   }
  // gemmi equivalent: static factory that takes a pointer to gemmi::Structure.
  // (We can't add another constructor to an already-defined class,
  //  so this is a static factory returning a value.)
  static dictionary_residue_restraints_t
  dictionary_residue_restraints_t_gemmi(const gemmi::Structure *st) {
    dictionary_residue_restraints_t result;
    result.filled_with_bond_order_data_only_flag = true; // nothing initially

    if (!st || st->models.empty()) return result;

    // Find the first residue (same logic as original: first model, first chain, first residue)
    const gemmi::Residue *first_residue = nullptr;
    for (const auto &chain : st->models[0].chains) {
      for (const auto &res : chain.residues) {
        first_residue = &res;
        break;
      }
      if (first_residue) break;
    }

    if (first_residue) {
      // Original called mol->RemoveBonds() then init(residue_p)
      // RemoveBonds has no gemmi equivalent (gemmi doesn't store bonds on structure)
      result.init_gemmi(first_residue);
    }
