...
struct dict_bond_restraint_t {
  ...
  dict_bond_restraint_t()
    : dist_(0), dist_esd_(0), order_(0), is_hydrogen_bond_(false), pdb_dist_(0), is_aromatic_(false) {}
...
public:
  ...
  std::vector<std::string> chiral_restraint;
  std::vector<std::string> plane_restraint;
  ...
  static std::string pad_name(const std::string& name) {
    ...
    return " " + no_space + std::string(std::max(0, 3 - static_cast<int>(no_space.size())), ' ');
  }
  ...
  void init_gemmi(const gemmi::Residue* residue_p) {
    ...
    for (unsigned int ibp = 0; ibp < static_cast<unsigned int>(bond_pairs.size()); ibp++) {
      for (unsigned int jbp = ibp; jbp < static_cast<unsigned int>(bond_pairs.size()); jbp++) {
        if (ibp != jbp) {
  ...
  // gemmi-port of the constructor dictionary_residue_restraints_t(mmdb::Residue* residue_p)
  // The original simply called init(residue_p).
  static dictionary_residue_restraints_t dictionary_residue_restraints_t_gemmi(const gemmi::Residue* residue_p) {
    dictionary_residue_restraints_t dict_res;
    dict_res.init_gemmi(residue_p);
    return dict_res;
  }
};
