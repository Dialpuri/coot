...
// ---- CXXChargeTable Implementation ----

int CXXChargeTable::addResidueMap(std::string theResidueName, stringDoubleMap theAtomCharges) {
    atomNameToChargeMap.insert(std::make_pair(theResidueName, theAtomCharges));
    return 0;
}

int CXXChargeTable::addChargeToResidueMap(stringDoubleMap& theResidueMap, std::string theAtomName, double theCharge) {
    theResidueMap.insert(std::make_pair(theAtomName, theCharge));
    return 0;
}
...
double CXXChargeTable::getCharge(std::string residueName, std::string atomName) {
    double charge = 0;
...
// ---- assignCharge_gemmi Implementation ----
...
namespace CXXUtils {
    int assignCharge_gemmi(std::vector<gemmi::CRA> selected, CXXChargeTable *theChargeTable, std::map<const gemmi::Atom*, double>& charges_out) {
        for (const auto& cra : selected) {
            if (!cra.atom || !cra.residue) continue;
            std::string atomName(cra.atom->padded_name());
            std::string residueName(cra.residue->name);
            double theAtomCharge = theChargeTable->getCharge(residueName, atomName);
            charges_out[cra.atom] = theAtomCharge;
        }
        return 0;
    }
}